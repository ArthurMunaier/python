...
@api_v1_bp.route("/sessoes", methods=["GET"])  # Rota para listar todas as sessões
...
def api_criar_sessao():
    ...
    # Status Code 201 = Created (Recurso criado com sucesso)
    return jsonify({
        "id": sessao.id,
...
def api_buscar_tmdb():
    # Utilizamos request.args.get() porque o termo da busca é enviado
    # como parâmetro na URL (Query String), por exemplo:
    # /api/v1/externo/filmes/buscar?q=Avatar
    # O método get() também evita erro caso o parâmetro não seja enviado,
    # retornando uma string vazia ("") como valor padrão.
